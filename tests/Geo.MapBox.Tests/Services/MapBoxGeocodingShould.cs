...
namespace Geo.MapBox.Tests.Services
{
    ...
    public class MapBoxGeocodingShould
    {
        ...
        [Test]
        public async Task CallMapBoxAsyncSuccessfully()
        {
            using var httpClient = new HttpClient(_mockHandler.Object);
            var service = new MapBoxGeocoding(httpClient, _keyContainer);
            var parameters = new ReverseGeocodingParameters()
            ...
            var uri = service.ValidateAndCraftUri<ReverseGeocodingParameters>(parameters, service.BuildReverseGeocodingRequest);
            var result = await service.CallMapBoxAsync<Response<Coordinate>>(uri).ConfigureAwait(false);
        ...
        public void CallMapBoxAsyncFailsWithException()
        {
            ...
            Action act = () => service.CallMapBoxAsync<Response<Coordinate>>(null).GetAwaiter().GetResult();

            act.Should()
                .Throw<MapBoxException>()
                .WithMessage("The MapBox uri is invalid. Please see the inner exception for more information.")
