...
namespace Geo.Trimble.Tests.DependencyInjection
{
    ...
    public class ServiceCollectionExtensionsTests
    {
        ...
        public void AddTrimbleGeocoding_WithValidCall_ConfiguresAllServices()
        {
            ...
            var options = provider.GetRequiredService<IOptions<KeyOptions<ITrimbleGeocoding>>>();
            options.Should().NotBeNull();
            options.Value.Key.Should().Be("abc");
            provider.GetRequiredService<ITrimbleGeocoding>().Should().NotBeNull();
        }
        ...
        public void AddTrimbleGeocoding_WithClientConfiguration_ConfiguresHttpClientAllServices()
        {
            ...
            // Assert
            var provider = services.BuildServiceProvider();
            var client = provider.GetRequiredService<IHttpClientFactory>().CreateClient("ITrimbleGeocoding");
