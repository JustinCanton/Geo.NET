...
namespace Geo.Core.Converters
{
        ...
        where T : Enum
    {
        ...
        public override T Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            {
                throw new JsonException(string.Format(CultureInfo.InvariantCulture, "Unexpected token while parsing the enum. Expected to find a string or number, instead found {0}", reader.TokenType.GetName()));
            }

            if (reader.TokenType == JsonTokenType.Number)
            {
                return (T)Enum.Parse(typeof(T), reader.GetInt32().ToString());
            }
